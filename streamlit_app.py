...
.stButton > button:focus {
    box-shadow: 0 0 0 2px #C0A062 !important;
...
musicals_data = [
    ...
    {
        "title": "김종욱 찾기", "genre": ["로맨스", "코미디"],
        "background_css": "linear-gradient(135deg, #ff8c00, #ffc837)",
        "summary": "인도 여행에서 만난 첫사랑 '김종욱'을 잊지 못하는 '그 여자'. 그녀는 아버지의 성화에 못 이겨 '첫사랑 찾아주기 사무소'를 찾아갑니다. 어딘가 어설픈 사무소장 '그 남자'와 함께 김종욱을 찾아 나서면서 벌어지는 유쾌하고 사랑스러운 로맨틱 코미디입니다. 한 명의 배우가 20개가 넘는 역할을 소화하는 '멀티맨'의 활약이 돋보입니다.",
        "cast": { "그 남자": ["엄기준", "오만석", "김무열", "유연석"], "그 여자": ["오나라", "김지우", "린아"] }
    },
    {
...
# 장르 목록 생성 (중복 제거 및 정렬)
all_genres = set()
for m in musicals_data:
    all_genres.update(m['genre'])
genres = ["전체"] + sorted(list(all_genres))

# 장르 버튼을 가로로 배치
num_columns = 8 # 한 줄에 표시할 버튼 수
rows = [genres[i:i + num_columns] for i in range(0, len(genres), num_columns)]
for row in rows:
    cols = st.columns(num_columns)
    for i, genre in enumerate(row):
        if cols[i].button(genre, key=f"genre_{genre}"):
            st.session_state.selected_genre = genre
            
# --- 검색 및 필터링 로직 ---
# 1. 장르 필터링
if st.session_state.selected_genre == "전체":
    genre_filtered_musicals = musicals_data
else:
    genre_filtered_musicals = [m for m in musicals_data if st.session_state.selected_genre in m['genre']]

# 2. 텍스트 검색 필터링
final_filtered_musicals = []
if search_term:
    search_term_no_space = search_term.replace(" ", "").lower()
    for musical in genre_filtered_musicals:
    ...
    final_filtered_musicals = genre_filtered_musicals
...
# --- 결과 출력 ---
# 검색을 하거나 장르를 선택했을 때만 결과가 나오도록 수정
if search_term or st.session_state.selected_genre != "전체":
    st.divider()
    if not final_filtered_musicals:
        st.warning(f"'{search_term or st.session_state.selected_genre}'에 대한 검색 결과가 없습니다.")
    else:
